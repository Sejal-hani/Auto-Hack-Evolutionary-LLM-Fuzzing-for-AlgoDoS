...
class LlamaFuzzerAgent:
    def __init__(self, api_key: str, model: str = "llama-3.3-70b-versatile"):
        if not api_key: raise ValueError("CRITICAL: API key is missing.")
        self.api_key = api_key
        self.model = model
        self.api_url = "https://api.groq.com/openai/v1/chat/completions"
        self.headers = {"Authorization": f"Bearer {self.api_key}", "Content-Type": "application/json"}

    def _build_system_prompt(self, ast_meta: CppAstMetadata, n_constraint: int, island_strategy: str, input_format: str) -> str:
        return f"""You are an elite automated vulnerability fuzzer. 
CONSTRAINT LOCK: The payload MUST use N = {n_constraint}.
ISLAND STRATEGY: {island_strategy}

{ast_meta.to_llm_prompt_context()}

CRITICAL: WRITE A PYTHON 3 SCRIPT that generates the input and prints it to standard output.
The printed output MUST STRICTLY follow this C++ format expectation:
{input_format}
...
{{
    ...
    async def generate_mutations(
        self, session: aiohttp.ClientSession, ast_meta: CppAstMetadata, 
        elite_telemetry: List[Dict[str, Any]], n_constraint: int,
        island_strategy: str = "DEFAULT", input_format: str = "N, then N integers.",
        healing_feedback: str = ""
    ) -> List[str]:
        system_prompt = self._build_system_prompt(ast_meta, n_constraint, island_strategy, input_format)
        
        user_content = "PREVIOUS GENERATION ELITE TEST CASES:\n"
        ...
        user_content += "\nWrite a Python 3 script to generate an even more destructive input. Output JSON with 'generator_code'."
        
        # SELF-HEALING INJECTION
        if healing_feedback:
            user_content += f"\n\n[CRITICAL ERROR TO FIX]: {healing_feedback}\nFix your Python code so this does not happen!"
