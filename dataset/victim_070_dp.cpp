...
struct segment_tree {
	...
	void pushdown(int p)
	{
		...
		tree[lc].mul = tree[lc].mul * tree[p].mul % mod; tree[lc].add = (tree[lc].add * tree[p].mul % mod + tree[p].add) % mod;
		tree[rc].sum = (tree[rc].sum * tree[p].mul % mod + tree[p].add * (tree[rc].r - tree[rc].l + 1) % mod) % mod;
		tree[rc].mul = tree[rc].mul * tree[p].mul % mod; tree[rc].add = (tree[rc].add * tree[p].mul % mod + tree[p].add) % mod;
...
//		std::cout << "  " << i << " " << idl << " " << L.query(1, 1, 1) << "\n";
	...
	for (int i = n; i >= 1; i--) {
		...
		if (a[i] == l[ll]) {
//			std::cout << " " << i << " " << L.query(1, idl - 1, idl - 1) << " " << R.query(1, 1, idr - 1) << " " << idl << " " << idr << "\n";
